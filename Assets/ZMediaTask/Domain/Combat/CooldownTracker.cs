...
namespace ZMediaTask.Domain.Combat
{
    ...
    {
        public bool IsReady(float currentTimeSec, float nextAttackTimeSec)
        {
            return currentTimeSec >= nextAttackTimeSec;
        }

        public float ComputeNextAttackTime(float currentTimeSec, int attackSpeed, float baseAttackDelay)
        {
            if (attackSpeed < 0)
            {
