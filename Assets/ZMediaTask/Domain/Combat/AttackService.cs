...
namespace ZMediaTask.Domain.Combat
{
    public sealed class AttackService
    {
            ...
            AttackConfig config)
        {
            ...
            if (failure.HasValue)
            {
                return new AttackResult(failure, attacker, target, 0);
            }
        ...
        private AttackFailureReason? GetFailureReason(
            CombatUnitState attacker,
            CombatUnitState target,
            float currentTimeSec,
            AttackConfig config)
        {
            if (!attacker.IsAlive)
            {
                return AttackFailureReason.AttackerDead;
            }

            if (!target.IsAlive)
            {
                return AttackFailureReason.TargetDead;
            }

            if (!IsInRange(attacker, target, config.AttackRange))
            {
                return AttackFailureReason.OutOfRange;
            }
